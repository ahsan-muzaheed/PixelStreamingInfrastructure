...
// // aldar pixel streaming urls
// let pixelstreamingurl = 'wss://aldar-staging.eaglepixelstreaming.com:443';
// // url to connect after user kicking someone
// let pixelstreamingurl1 = 'wss://aldar-staging.eaglepixelstreaming.com:443';
// // url if first slot is occupied
// let pixelstreamingurl2 = 'wss://broadcast1.eaglepixelstreaming.com:443';

// ruya pixel streaming urls
// let pixelstreamingurl = 'wss://broadcast1.eaglepixelstreaming.com:443';
// pixelstreamingurl = 'ws://99.60.91.140:80';
//url to connect after user kicking someone
// let pixelstreamingurl1 = 'wss://broadcast1.eaglepixelstreaming.com:443';
//url if first slot is occupied
// let pixelstreamingurl2 = 'wss://aldar-staging.eaglepixelstreaming.com:443';

// // aws streaming urls
// let pixelstreamingurl = 'wss://aldardemo.eaglepixelstreaming.com:443';
// // url to connect after user kicking someone
// let pixelstreamingurl1 = 'wss://aldardemo.eaglepixelstreaming.com:443';
// // url if first slot is occupied
// let pixelstreamingurl2 = 'wss://aldardemo2.eaglepixelstreaming.com:443';
let mmResponse = null;
let updateAvailable = false;
let appAcquiringData = 0;
let appPreparationData = 0;
let firstLoad = true;
let secondLoad = true;
let kicked = false;
let playersCountGlobal = -1;
let playersCountFlag = true;
let needToKick = false;
let connectedAtLeastOnce = false;
let needToResetStreamer = false;
let stopWs = false;
...
let print_inputs = false; //prints screen coords to console
let connect_on_load = false;

let is_reconnection = false;
let ws_error = false;
let streamer_time_out = false;
let ws;
...
function addResponseEventListener(name, listener) {
	//host page adds uniquely named callbacks to receive UE json objects
...
function setupWebRtcPlayer(htmlElement, config) {
	...
	webRtcPlayerObj.onDataChannelMessage = function (data) {
		...
		if (view[0] === ToClientMessageType.QualityControlOwnership) {
			...
			// console.log("111111111 ue4 response: " + response); //webrtc forwards message from UE to js listeners
...
// 		styleTop = 0;
// 		styleLeft =
// 			window.innerHeight <= styleHeight + headerHeight
// 				? -(styleWidth - styleHeight / videoAspectRatio) / 2
// 				: 0;
...
// 	const newHeight = Math.floor(window.innerWidth * videoAspectRatio);
// 	styleHeight =
// 		newHeight > window.innerHeight - headerHeight
// 			? window.innerHeight - headerHeight
// 			: newHeight;
// 	if (styleHeight + headerHeight - window.innerHeight > 100) {
// 		styleWidth = window.innerWidth;
// 		styleTop = 0;
// 		styleLeft = -(styleWidth - styleHeight / videoAspectRatio) / 2;
// 	} else {
// 		styleWidth = (window.innerHeight - headerHeight) / videoAspectRatio;
// 		styleHeight = window.innerHeight - headerHeight;
// 		styleLeft = 0;
// 	}
// 	//Video is now 100% of the playerElement, so set the playerElement style
...
function connect(psUlr) {
	...
	ws.onmessage = function (event) {
		...
		if (msg.type === 'config') {
			...
			/* if (playersCountFlag)
      {
        playersCountFlag = false;
        playersCountGlobal = msg.count;
      }
      if (msg.count === 1 && (firstLoad || connectedAtLeastOnce))
      {
        needToResetStreamer = true;
      }
      if (msg.count > 1 && firstLoad && psUlr === psUlr1) {
        console.log(
          `Changing psUlr ${psUlr1} to ${psUlr2}`,
        );
        psUlr = psUlr2;
        ws.close();
      } else if (
        ...
        secondLoad = false;
        ws.close();
        window.location.href = window.location.origin + '/#/login?noPlaces=true';
      }
      firstLoad = false;
