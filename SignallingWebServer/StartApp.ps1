...
Param (
    ...
    # [int] $instanceNum = 6,
     ...
     # [Parameter(Position=1, Mandatory=$false, HelpMessage = "The name of the 3D app, default PixelStreamingDemo")]
      #[string] $pixel_stream_application_name = "EagleStreamingMechns" #,
	# [Parameter(Position=1, Mandatory=$false, HelpMessage = "")]
    # [string] $PixelStreamerExecFile = "EagleStreamingMechns",
	#[Parameter(Position=2, Mandatory=$false, HelpMessage = "")]
    #[string] $args = ""
	
)

 Write-Output "******************PixelStreamerExecFile :" + $PixelStreamerExecFile
 Write-Output "******************streamingPort :" + $streamingPort
foreach($var in (get-variable -scope private))
    {
        #write-host "$($var.name) -> $($var.value)"
    }
	
#$PixelStreamerExecFile = "C:\0.apps_azure\ruya\EagleStreamingMechns\18\EagleStreamingMechns.exe"


$args=$pixel_stream_application_name# "-log -RenderOffScreen -AudioMixer -PixelStreamingEncoderMaxQP=31   -PixelStreamingIP=localhost -PixelStreamingPort=8888"	
		
#####################################################################################################
#base variables
#####################################################################################################
$PixelStreamerFolder = "C:\0.apps_azure\ruya\EagleStreamingMechns\18\"
#$PixelStreamerExecFile = $PixelStreamerFolder + $pixel_stream_application_name + ".exe"
$vmServiceFolder = "C:\0.ps\azurepsepic\SignallingWebServer\"
...
$streamingPortArg = "-PixelStreamingPort=" + $streamingPort
...
Write-Output "PixelStreamerFolder :" + $PixelStreamerFolder
...
try {
& $PixelStreamerExecFile $audioMixerArg $streamingIPArg $streamingPortArg $renderOffScreenArg -Unattended -log -WinX=0 -WinY=0 $resolutionWidthArg $resolutionHeightArg -Windowed
#& $PixelStreamerExecFile $args
...
Write-Output "vmServiceFolder :" + $vmServiceFolder
